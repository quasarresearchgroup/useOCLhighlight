...
public interface IComplexityMetricResult {
	 ...
	 * defined as ∑ ( 1 + | Par(m) | ) (1+ R + |Pout, in/out (m)|) m ∈ N(expression),
	 * where:
	 * ● N(expression): Set of different operations referred through navigations.
	 * ● |Par(m)|: quantity of actual parameter of m. R stand for
	 * the m operation result and represents the value of 1. 
	 * ● |Pout, in/out (m)|: quantity of out and in/out parameter of the m 
	 * operation. Through a navigation it is possible to exclusively access 
	 * one result as a time, this weighted formula giving a higher weight 
	 * to those operations used many times to access different results.
	 */
	int getWNO();
	void setWNO(int wno);
	 ...
	 * according to: ∑ ( 1 + | Par(m) | ) m ∈ M(expression), where:
	 * ● M(expression): Set of different operations2 used through messaging in an expression.
